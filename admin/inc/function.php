...
function ajax_list_data($data){
    ...
    $total=$mysqli->affected_rows;
    $pagenum=ceil($total/$page_size);
    if($data['page']!="all"&&($data['page']<1||!is_numeric($data['page'])||$data['page']>$pagenum))$data['page']=1;
...
function ajax_data_num($data){
    global $mysqli;
    $sql="select count(*) as count from ".$data['from'];
    //$sql.=" where DATE_FORMAT(FROM_UNIXTIME(rq),'%Y-%m-%d') = DATE_FORMAT(NOW(),'%Y-%m-%d')";
    if($data['from']=='spider'){
        ...
        if($data['day']){
            $sql.=" where DATE_FORMAT(FROM_UNIXTIME(rq),'%Y-%m-%d') = '".date('Y-m-d',strtotime($data['day']))."'";
        }
        if($data['type']){
            $sql.=" where ssyq='".$data['type']."'";
        }
